...
int generate_trace_entry_tid(struct bpf_insn **instructions, size_t *size, int tid, int fd3) {
  *size = 32 * sizeof(struct bpf_insn);
  *instructions = malloc(*size);
  if (instructions == NULL) {
    return -1;
  }
...
int generate_trace_entry_pid(struct bpf_insn **instructions, size_t *size, int pid, int fd3) {
  *size = 35 * sizeof(struct bpf_insn);
  *instructions = malloc(*size);
  if (instructions == NULL) {
    return -1;
  }
...
int generate_trace_return(struct bpf_insn **instructions, size_t *size, int fd3, int fd4) {
  *size = 82 * sizeof(struct bpf_insn);
  *instructions = malloc(*size);
  if (instructions == NULL) {
    return -1;
  }
